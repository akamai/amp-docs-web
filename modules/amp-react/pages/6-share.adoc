...
Player offers `share` event callback to hook users choice, within in payload can be use to perform an action such like prompting a redirect to the social network url or show a custom overlay.  
[source,javascript]
----
...
If required, the list of providers listed in the share panel UI element can be modified whether to add certain options or remove unneeded options on `providers` array.
